---
keywords: [permissions, escalation, policy]
related: [50_permissions/01_consent-economy.md, 40_delegation/02_routing-rubric.md, 40_delegation/03_delegation-contract.md, 50_permissions/03_blast-radius-rubric.md, 25_writing-style/06_user-care/02_first-response-protocol.md]
---
...
2. **同じ訂正が 2 回**: ユーザーが同じ指示を 2 回繰り返した → 進行中タスクを停止 ([25_writing-style/06_user-care/](../25_writing-style/06_user-care/))。
...
| Escalation L2 (同じ訂正 2 回) | **全停止**。阻害要因除去に集中 ([25_writing-style/06_user-care/02_first-response-protocol.md](../25_writing-style/06_user-care/02_first-response-protocol.md)) |
...
| 内部権限ステータス (ASK/ALLOW/保留) をユーザー出力に露出する (例:「push は ASK 該当のため保留しています」) | ユーザーは権限機構の運用責任を持たない。ハーネスの内部状態を渡すと判断委譲 + 承認疲労が起きる。事前承認スコープ ([02_pre-authorized-actions.md](./02_pre-authorized-actions.md) cli-git-remote 等) に該当する操作は agent 判断で実行し、Escalation L2 として扱わない |
| 通常の git 操作 (push 含む) を毎回ユーザーへ委譲する | 「Push するかしないか」は方針判断ではなく作業遂行。事前承認スコープ内であれば agent が実行責任を持つ。Escalation L2 は **方針分岐 / 不可逆 / 北極星に直結** する判断のみに限定する |

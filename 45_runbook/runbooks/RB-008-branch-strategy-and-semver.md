...
- `main` = 配布版。`.handover/` 等の開発資産は **入らない** (`.gitignore` で除外)
- `develop` = 開発版。`.handover/` を追跡対象として保持
...
|---|---|---|
| 利用者の clone 体験 | `.handover/` 等のノイズが混入 | クリーンな配布版 |
...
- .gitignore (.handover/ や _tmp/ を除外)
- bootstrap.sh

含まないもの:
- .handover/ (開発進捗、develop でのみ追跡)
- _tmp/ (検証中間ファイル)
- IDE 設定 / OS 一時ファイル
```
...
```
main の全内容 + 以下を追加:
- .handover/ (開発用の GOAL / PENDING / current_session / HISTORY / COMPLETED)
- 検証スクリプト / 一時的なメモ
```
...
[2] develop に commit + push (.handover/ 含む)
[3] リリース判定 (RB-008 §リリース判定基準) を満たしたら:
    [3.1] develop の中で .handover/ 以外を整理
    [3.2] git checkout main && git merge develop --no-ff
    [3.3] .gitignore で .handover/ を除外しているので main には自動的に含まれない
...
- 利用者の既存 `.handover/` 互換性を保つ (schema 変更は major bump)
...
`develop` branch では `.handover/` を **除外しない** (`.gitignore` 内の `.handover/` 行をコメントアウトするのではなく、merge 戦略で main の `.gitignore` を develop に持ち込んだ上で、`.handover/` は develop では既に commit 済 = 追跡対象として残る)。

git の挙動: `.gitignore` は **未追跡ファイルを ignore する** ものなので、既に追跡対象のファイルには影響しない。develop で `.handover/` を最初に commit しておけば、後から main の `.gitignore` を merge しても `.handover/` は追跡継続される。
...
[6] .gitignore で .handover/ が除外されていることを確認
...
- [ ] main を clone した利用者の作業ディレクトリに `.handover/` が存在しない
- [ ] develop を clone した開発者の作業ディレクトリに `.handover/` が存在する
...
| 症状 | 原因 | 対処 |
|---|---|---|
| main に `.handover/` が混入 | `.gitignore` 設定漏れ or git rm 忘れ | `git rm -rf --cached .handover/` + `.gitignore` 確認 + commit |
| develop で `.handover/` が無視される | 既に追跡対象だが checkout タイミングで欠落 | `git checkout develop -- .handover/` |
...
| develop で `.handover/` を `.gitignore` 対象にする | 開発進捗が永続化されず引き継ぎ不能 |
...
- RB-006 (セッション引き継ぎ、`.handover/` の役割定義)
- RB-007 (1 セッション 1 タスク、`.handover/` の運用規律)

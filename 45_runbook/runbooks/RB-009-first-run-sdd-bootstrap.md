...
keywords: [runbook, runbooks, first, run, sdd, bootstrap]
---
...
Claude Code / Claude Agent SDK は **ターン駆動 (turn-driven)**。VSCode でフォルダを開く・
`claude` を起動するだけでは agent は発火しない。**ユーザーの最初のメッセージで初めて 1 ターン目が走る**。

したがって本 Runbook の「自動開始」は、

- × フォルダを開いたら勝手に質問が出てくる
- ○ ユーザーが最初の入力 (`開始` / `.` / 具体タスク など、内容は問わない) を送った瞬間、
     その 1 ターン目で agent が `.session-state/` 不在を検出し、追加の確認なしにヒアリングを開始する

の意味。ユーザーが打ち込むべき文字列は何でもよい (空でない 1 メッセージ)。
配布利用者には「フォルダを開いたあと、何でもいいので 1 通送ってください」と案内する。

## 初回フロー (擬似コード)

```
on_first_user_message():           # ← セッション起動ではなく "最初のユーザー入力" で発火
...
- [ ] 配布版 main を clone した利用者の作業ディレクトリで agent 起動 + 最初のユーザー入力 → 同一ターンでヒアリング 1 問目が返る
- [ ] ヒアリング 5 質問以下で完結
...
| ユーザーが「フォルダを開いたが何も聞かれない」と報告 | ターン駆動モデルの誤認 | 「何でもよいので 1 通送ってください (例: `開始`)」と案内。ハーネス側のバグではない |
...
| 「フォルダを開いた瞬間に agent が話しかけてくる」と説明 | ターン駆動モデル違反のハルシネーション。利用者は「打ったのに動かない」と離脱 |

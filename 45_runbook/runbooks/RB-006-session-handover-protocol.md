...
セッション間引き継ぎを **agent が完全自動化** する規律。ユーザーに「次回プロンプトをコピペ」「宿題を覚えておく」を要求しない (RB-003 / 01_overview/05_user-as-hands.md 整合)。仕組み: `45_runbook/_handover/PENDING.md` を SSOT として、**開始時に Read、終了時に Write** を Orchestrator system prompt に必須化。
...
```
~/.claude/methods/ecc-method/45_runbook/_handover/
...
| _handover/ ディレクトリが空 | 初期化忘れ | bootstrap.sh に `mkdir -p _handover && touch _handover/{PENDING,COMPLETED,HISTORY}.md` を追加 |

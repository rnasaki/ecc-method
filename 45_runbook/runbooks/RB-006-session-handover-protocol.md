...
## ユーザーが取るべき行動 (= 1 メッセージのみ)

Claude Code / Claude Agent SDK は **ターン駆動 (turn-driven)**。
セッションを起動した時点では agent は発火しない。**ユーザーが最初の 1 メッセージを送って初めて 1 ターン目が走る**。
詳細は [[RB-009-first-run-sdd-bootstrap]] §起動トリガーの前提。

### 通常時 (継続モード)

1. 新セッションを起動 (Claude Code セッション開始)
2. **任意の 1 メッセージを送る** (例: `再開` / `.` / 具体タスク追加 等、内容は問わない)
3. agent が 1 ターン目で `.session-state/` を Read して自動継続

### 初回利用時 (初回モード)

1. 配布版 (`main`) を clone した直後、案件リポで agent を起動
2. **任意の 1 メッセージを送る** (例: `開始` / `.`、内容問わず)
3. agent が 1 ターン目で `.session-state/` 不在を検出し SDD ヒアリング 5 質問を開始 (RB-009)
4. 回答内容から `.session-state/` 5 ファイルが自動生成される

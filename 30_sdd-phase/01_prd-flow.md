...
- `60_quality-gates/` の Gate 1 (要件完成) と連動する。
- レビュアは「背景と機能の対応」「スコープ境界の明確さ」「指標の計測可能性」を中心に確認する。

## 4. 受け渡し形式

- 配置: リポジトリ直下の `specs/<feature-set>/PRD.md`
- 文字コード: UTF-8 (BOM 無し)
- 改行: LF
...
- 成功指標の合格しきい値はプロジェクト個別事情で変動する。本書は「定量化必須」のルールのみ規定し、絶対値は規定しない。
- 技術スタックの選定基準は組織のスキルセットに依存するため、本書では「既存採用優先」までを規定する。
